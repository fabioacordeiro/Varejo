...
def criar_apresentacao_sinistro(numero_sinistro=None, excel_file=None, template_path=None):
    ...
    # Imprimir as colunas e tipo de dados do dataframe
    print(58*'-')
    print(sinistro_data.dtype)
    print(58*'-')
...
# Formatar a data do sinistro
    ...
    #dt_encosta_em_doca = sinistro_data.get("ENCOSTA_EM_DOCA", "N/A")
    #sinistro_data["categorical"] = sinistro_data["object"].astype("category")
...
# --- Função nova para normalizar datas ---
def format_datetime(value):
    ...
    if isinstance(value, (int, float, np.floating)):
        try:
            dt = datetime(1899, 12, 30) + timedelta(days=float(value))  # serial Excel
            return dt.strftime("%Y-%m-%d %H:%M")
        except Exception:
            return ""

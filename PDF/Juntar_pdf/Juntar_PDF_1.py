...
try:
    ...
    for i in arquivos:
        ...
        if extensoes == []:
            ...
            extensao = i.split('.')[-1]
            print(f'Extensão captada:{extensao}')
            if extensao in extensoes:
                print(f'ver extensao:',(extensao))
                print(f'Nome do arquivo:{extensoes}')
                lista_inicial = str(i)
                lista_pdf.append(lista_inicial)
            else:
                print(f'Este não:',(extensao))
    print(f'Lista: {lista_pdf}')
    for pdf in (lista_pdf):
        if pdf == pdf[0]:
            print('Este é o primeiro arquivo')
        else:
    ...
    with open(PASTA_ALTERADOS/'Juntar.pdf', 'wb') as m:
        merger.write(m)
    merger.close()
    lista_pdf = list() #zerar as listas
    lista_inicial = list() #zerar as listas
    print('Fim')

...
for transportadora in df_dados["TRANSPORTADORA"].unique():
    ...
    # Criar corpo do e-mail com as linhas da transportadora
    corpo_email = """ {email_transp}
...
"""
    ...
    # Obter os destinatários únicos
    emails = df_filtro["EMAIL"].dropna().unique()
    email_transp = df_filtro["Email_Transp"].dropna().unique()

    # Enviar e-mails
    for email in emails:
        enviar_email(email, transportadora, ", ".join(email_transp), corpo_email)

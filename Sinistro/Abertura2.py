...
def enviar_email(destinatario: str, assunto: str, corpo_html: str) -> bool:
    ...
    # 1) Tenta SMTP STARTTLS (porta 587)
    try:
    ...
    except (socket.timeout, TimeoutError, ConnectionRefusedError, OSError, smtplib.SMTPException) as e587:
        print(f"Falha ao enviar e-mail via 587 (STARTTLS): {e587}")

    # 2) Fallback: SMTP SSL (porta 465)
    try:
        with smtplib.SMTP_SSL(SMTP_SERVER, 465, timeout=20, context=ssl.create_default_context()) as server:
            server.login(FROM_EMAIL, EMAIL_PASSWORD)
            server.sendmail(FROM_EMAIL, destinatario, msg.as_string())
        return True

    except (socket.timeout, TimeoutError, ConnectionRefusedError, OSError, smtplib.SMTPException) as e465:
        print(f"Falha ao enviar e-mail via 465 (SSL): {e465}")
...
"""
    ...
    ok = enviar_email(DESTINATARIO_EMAIL, assunto, corpo_email)
    print("Enviado (e-mail)" if ok else "E-mail não enviado (processo continuou e o Excel foi gerado).")
